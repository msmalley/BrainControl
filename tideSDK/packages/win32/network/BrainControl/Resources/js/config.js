	...
	this.settings = function(options)
	{
		...
		options.panels.push({
			...
			middle: {
				...
				form: [
					...
					{ id: 'url', type: 'text', label: base.lang('URL Used'), placeholder: base.lang('Where was the original wallet?') },
					{ id: 'pin', classes: 'pin_number' },
				]
			},
			bottom: {
		...
		options.panels.push({
			...
			middle: {
				...
				paragraphs: [
					{ text: base.lang('This allows you to import private keys generated from any device or service. Please note it uses the device salt when importing.'), classes: 'unessential' },
					{ text: base.lang('Please also note in order to import the keys to this device we actually have to transfer the ocntents from that account to a new one created during the import process with a network transaction fee of 0.0001.'), classes: 'unessential' }
				]
			},
			bottom: {
				buttons: [
		...
		options.panels.push({
			...
			bottom: {
				buttons: [
					{ href: '#', classes: 'bc-icon bc-action bc-half bc-pin bc-slide hidden', text: base.lang('add pin'), icon: 'plus-circle', direction: 'up', id: 'step-pin', back_direction: 'down', back_id: 'step-confirm' },
